...
(function () {
	...
	function generateDayCalendar(calendarEl, calendarEvents) {
		...
		var calendar = new FullCalendar.Calendar(calendarEl, {
			...
			eventClick: function (info) {
				...
				if (
					isVisible &&
					eventDetailsContainer.dataset.eventId === info.event.id
				) {
	...
	function showDetails(event) {
		...
		if (
			...
			eventDetailsContainer.classList.contains("visible")
		) {
			...
			var detailedEvents = getDetailedEventsForTimeBlock(
				event.start,
				event.end
			);
	...
	function processStorage(callback) {
		chrome.storage.local.get("tabFocusEvents", function (result) {
			...
			for (const domain in tabFocusEvents) {
				if (
					tabFocusEvents.hasOwnProperty(domain) &&
					Array.isArray(tabFocusEvents[domain].events)
				) {
